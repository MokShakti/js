...
//Умова: Дано інформацію про прибуток К магазинів протягом тижня. Знайти: Максимальний прибуток за середу.
...
if (confirm(`Shall we start the test?`)) {
   ...
   for (let weekNum = 0; weekNum < income.length; weekNum++) {
      for (let dayNum = 2; dayNum <= 2; dayNum++) {
         dayIncome += income[weekNum][dayNum]
      }
   }
   document.write(`Total sum for Wednesday is: ${dayIncome}`)
}